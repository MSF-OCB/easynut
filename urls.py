...
from RESTViews import TableList, TableDetail
...
urlpatterns = [
    ...
    url(r'^api/(?P<table_id>[0-9]+)/$', TableList.as_view()),
    url(r'^api/(?P<table_id>[0-9]+)/(?P<pk>(NULL|[0-9]+))/$', TableDetail.as_view()),
	...
	url(r'^(?P<table_id>[0-9]+)/(?P<related_record_entry>(NULL|[0-9]+))/(?P<related_record_field>[a-zA-Z_0-9]+)/addrecord/$', views.addrecord, name='addrecord'),

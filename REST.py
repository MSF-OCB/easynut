...
class TableSerializer(serializers.Serializer):
    ...
    def __init__(self, table_config, showall=True, **kwargs):
        ...
        for column in fields:
            if column.type == FieldConfig.field_type_int:
                self.fields[self.sanitize(column.name)] = serializers.IntegerField(source=column.field)
            elif column.type in (FieldConfig.field_type_str, FieldConfig.field_type_not, FieldConfig.field_type_sel):
                self.fields[self.sanitize(column.name)] = serializers.CharField(source=column.field)
            elif column.type == FieldConfig.field_type_date:
                self.fields[self.sanitize(column.name)] = serializers.DateTimeField(source=column.field)
    ...
    def create(self, validated_data):
        return Table(id=None, table_config=self.table_config, **validated_data)
